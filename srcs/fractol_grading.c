...
/*   Updated: 2025/02/19 15:48:52 by liovino          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_colour	colour_render(int i, t_palette *palette, t_fractal *fractal)
{
	...
	index = (int)(keep_i);
	i_step = (keep_i - index);
	return (blend(ft_itoc(palette->colours[index]), \
		ft_itoc(palette->colours[index + 1]), i_step));
}
...
int	get_colour(t_complex z, int i, t_fractal *fractal)
{
	...
	else
	{
		if (fractal->palette.val == 1)
			f_colour.val = scaling(i, fractal->image_def, YELLOWISH, LILAC);
		else
			f_colour.val = scaling(i, fractal->image_def, DEEP_BROWN, \
									WARM_CREAM);
	}
	return (f_colour.val);
}
